...
module.exports = (prisma) => {
    ...
    router.post('/register', async (req, res) => {
        ...
        try {
            ...
            if (securityQuestion === "Escolha uma pergunta de segurança") {
                return res.status(400).send('Por favor, escolha uma pergunta de segurança válida.');
            }
            ...
            await prisma.user.create({
                data: {
                    ...
                    rpg: {
                        create: {
                            // CORREÇÃO: Adiciona os campos que estavam faltando para evitar erros de 'null'.
                            level: 1,
                            xp: 0,
                            xpToNextLevel: 100,
                            coins: 10,
    ...
    router.post('/recover/get-question', async (req, res) => {
        ...
        if (!user || !user.securityQuestion) return res.status(404).send('Nenhuma conta encontrada com este email ou nenhuma pergunta de segurança configurada.');
    ...
    router.post('/recover/validate-answer', async (req, res) => {
        ...
        const user = await prisma.user.findUnique({ where: { email } });
        if (!user || !user.securityAnswerHash || !bcrypt.compareSync(answer, user.securityAnswerHash)) return res.status(401).send('Resposta de segurança incorreta.');
